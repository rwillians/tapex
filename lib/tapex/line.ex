defmodule Tapex.Line do
  ...
  def format_line(%{} = test, number, colorize) do
    ...
    (format_line_status(ok, colorize) <> format_line_number(number))
  ...
  defp format_line_directive(:skip, message, colorize) do
    ("# " <> color_wrap("SKIP", :yellow, colorize))
    |> spacecat(message)
  end

  defp format_line_directive(:todo, message, colorize) do
    ("# " <> color_wrap("TODO", :blue, colorize))
    |> spacecat(message)
  end

  defp get_directive(%{tags: %{skip: true}}), do: {:skip, nil}
  defp get_directive(%{tags: %{skip: reason}}) when is_binary(reason), do: {:skip, reason}
  defp get_directive(%{state: {:skip, reason}}), do: {:skip, reason}
  defp get_directive(%{tags: %{todo: true}}), do: {:todo, nil}
  defp get_directive(%{tags: %{todo: reason}}) when is_binary(reason), do: {:todo, reason}
  defp get_directive(%{}), do: {nil, nil}
