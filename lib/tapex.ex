defmodule Tapex do
  ...
  def init(opts) do
    ...
    colorize =
      case get_in(opts, [:colors, :enabled]) do
        nil -> IO.ANSI.enabled?()
        enabled -> enabled
      end
  ...
  defp increment_type_counter(%{} = config, %{}), do: config
  ...
  defp increment_state_counter(%{} = config, %{}), do: config
